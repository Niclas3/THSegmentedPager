...
- (void)viewDidLoad {
    ...
    if ([self isConformProtocolAndRespondsSelector]) {
        [_delegate settingPageControll];
    } else {
...
- (BOOL)isConformProtocolAndRespondsSelector {
    return [self.delegate conformsToProtocol:@protocol(THSegmentedPageViewControllerDelegate)] && [self.delegate respondsToSelector:@selector(settingPageControll)];
